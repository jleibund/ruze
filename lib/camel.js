...
//        if (ep.prior.prior) delete ep.prior.prior;
    ...
    route.fn.endpoint = function(instance) {
        ...
        if (route && route.length > 0){
            ...
            if (id)
                camel.endpoints[id] = {instance:instance, config:ep};
        }
        return this;
    }
     ...
     */
        ...
        camel.fn.endpoint = function(end,cb){
            ...
            return r.build().then(function(){
                cb(camel.endpoints[r.route[0].id].instance);
            });
        }
        ...
        camel.fn.print = function () {
            ...
            console.log('\t',_.keys(this.cps));
            console.log('\nActive Endpoints:');
            _.each(this.endpoints, function(ep,key){
                console.log('\t(',key,')');
                if (ep.config.recipientList){
                    console.log('\t\tfires ',_.pluck(ep.config.recipientList,'id'));
                } else {
                    console.log('\t\tfires nothing')
                }
                var count = 0;
            ...
            _.each(this.routes, function(r){
                ...
                console.log('\t\t sequence ', _.pluck(r.route,'id'));
                console.log('\t\t built ', r.built);
            },this)
            console.log('\nActive Loaders:');
            _.each(this.loaders, function(l,key){
                console.log('\t',key,'paths : ',l.paths);
            })
        }
