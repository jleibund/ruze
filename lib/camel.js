...
define([ 'node-uuid', 'events', 'underscore', 'q'], function(uuid, events, _, Q){
    ...
    // todo - exception references should be added to exchanges, also should contain the from endpoint at each stage
...
//        var previousCamel = root.camel;
        ...
        // we are removing singleton pattern
...
//            var name = endpoint.component, container = endpoint.container;
            ...
            if (comp) return Q.fcall(function(){return comp}).then(config);
...
//                if (comp) return Q.fcall(function(){return 'cream'});
            ...
            return look(endpoint).then(config);
        };
