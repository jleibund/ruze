...
define( function () {
    ...
    // should support nesting e.g. '${properties:${in.header.somekey}}
    ...
    var _ref = {keyword:'ref', beforeExpr:true},
        _last = {keyword:'last'},
        _bodyAs = {keyword:'bodyAs'};

    var _null = {keyword: 'null', atomValue: null}, _true = {keyword: "true", atomValue: true},
        _false = {keyword: 'false', atomValue: false};

    var keywordTypes = {'null':_null, 'true':_true, 'false':_false, 'ref':_ref, 'last':_last, 'bodyAs':_bodyAs};
    ...
    var nativeOperators = ['==','>','>=','\<=','!=','===','!==='];  // defines lefthand/righthand size of comparison
    var nonnativeOperators = ['contains','not contains','regex','not regex','in','not in','is', 'not is', 'range', 'not range', 'typeof', 'not typeof'];
    var unaryOperators = ['++','--'];
    ...
    var isKeyword = makePredicate("null true false instanceof typeof");
     ...
     */
    var Parser = function(expr, refs, obj1, obj2) {
    ...
    Parser.prototype.raise = function(pos, message){
        ...
        err.pos = pos; err.raisedAt = this.tok.pos;
        throw err;
    }
    ...
    Parser.prototype.readToken_slash = function() {
        var tok = this.tok, input = this.input;
        var next = input.charCodeAt(tok.pos+1);
        if (tok.regexp) {++tok.pos; return readRegexp();}
    ...
    Parser.prototype.readToken_lt_gt = function(code) {
        ...
        if (next === code) {
            ...
            if (input.charCodeAt(tokPos + size) === 61) return this.finishOp(_assign, size + 1);
    ...
    Parser.prototype.getTokenFromCode = function(code) {
        ...
        switch(code) {
            ...
            case 40: ++tok.pos; return this.finishToken(_parenL);
            case 41: ++tok.pos; return this.finishToken(_parenR);
            case 44: ++tok.pos; return this.finishToken(_comma);
            case 91: ++tok.pos; return this.finishToken(_bracketL);
            case 93: ++tok.pos; return this.finishToken(_bracketR);
            case 123: ++tok.pos; return this.finishToken(_braceL);
            case 125: ++tok.pos; return this.finishToken(_braceR);
            case 58: ++tok.pos; return this.finishToken(_colon);
            case 63: ++tok.pos; return this.finishToken(_question);
...
//        if (options.locations) tokStartLoc = new line_loc_t;
    ...
    Parser.prototype.readHexNumber = function() {
        ...
        if (val == null) this.raise(tokStart + 2, "Expected hexadecimal number");
...
//        else if (/[89]/.test(str) || strict) this.raise(start, "Invalid number");
    ...
    Parser.prototype.readString = function(quote) {
        var tokPos = this.tok.pos, tokStart = this.tok.start, input = this.input, inputLen = this.inputLen;
        tokPos++;
        var out = "";
        for (;;) {
            if (tokPos >= inputLen) this.raise(tokStart, "Unterminated string constant");
...
//                            break;
                ...
                if (ch === 13 || ch === 10 || ch === 8232 || ch === 8329) this.raise(tokStart, "Unterminated string constant");
    ...
    Parser.prototype.readWord1 = function() {
        var tokPos = this.tok.pos, input = this.input, inputLen = this.inputLen;
        this.containsEsc = false;
        var word, first = true, start = tokPos;
        for (;;) {
            var ch = input.charCodeAt(tokPos);
            if (isIdentifierChar(ch)) {
                if (this.containsEsc) word += input.charAt(tokPos);
                ++tokPos;
            } else if (ch === 92) { // "\"
                if (!this.containsEsc) word = input.slice(start, tokPos);
                this.containsEsc = true;
                if (input.charCodeAt(++tokPos) != 117) // "u"
                    this.raise(tokPos, "Expecting Unicode escape sequence \\uXXXX");
                ++tokPos;
                var esc = this.readHexChar(4);
                var escStr = String.fromCharCode(esc);
                if (!escStr) this.raise(tokPos - 1, "Invalid Unicode escape");
                if (!(first ? isIdentifierStart(esc) : isIdentifierChar(esc)))
                    this.raise(tokPos - 4, "Invalid Unicode escape");
                word += escStr;
            } else {
                break;
            }
        ...
        return this.containsEsc ? word : input.slice(start, tokPos);
    }
...
//                    semicolon();
    ...
    Parser.prototype.parseMaybeAssign = function(noIn) {
        var left = this.parseMaybeConditional(noIn), tokType = this.tok.type, tokVal = this.tok.val;
        if (tokType.isAssign) {
    ...
    Parser.prototype.parseExprOp = function(left, minPrec, noIn) {
        var tok = this.tok, tokType = tok.type, tokVal = tok.val, prec = tokType.binop;
        if (prec != null && (!noIn || tokType !== _in)) {
            if (prec > minPrec) {
                ...
                var node = this.finishNode(node, /&&|\|\|/.test(node.operator) ? "LogicalExpression" : "BinaryExpression");
    ...
    Parser.prototype.parseMaybeUnary = function(noIn) {
        var tok = this.tok, tokType = tok.type, tokVal = tok.val;
        if (tokType.prefix) {
            var node = this.startNode(), update = tokType.isUpdate;
            node.operator = tokVal;
            node.prefix = true;
            this.next();
...
//        while (tokType.postfix && !canInsertSemicolon()) {
    ...
    Parser.prototype.parseExprAtom = function() {
        var tok = this.tok, tokType = tok.type, tokVal = tok.val, tokStart = tok.start, tokEnd = tok.end;
        switch (tokType) {
//            case _this:
//                var node = startNode();
//                next();
//                return finishNode(node, "ThisExpression");
            ...
            node.value = tokVal;
            node.raw = this.input.slice(tokStart, tokEnd);
            this.next();
...
//            }
    ...
    Parser.prototype.parsePropertyName = function() {
        var tokType = this.tok.type;
        if (tokType === _num || tokType === _string) return this.parseExprAtom();
        return this.parseIdent(true);
    }

    Parser.prototype.parseExprList = function(close, allowTrailingComma, allowEmpty) {
//    function parseExprList(close, allowTrailingComma, allowEmpty) {
        var elts = [], first = true, tokType = this.tok.type;
...
//                if (allowTrailingComma && options.allowTrailingCommas && eat(close)) break;
    ...
    Parser.prototype.parseIdent = function(liberal) {
        var node = this.startNode(), tokType = this.tok.type, tokVal = this.tok.val;
//        node.name = tokType === _name ? tokVal : (liberal && !options.forbidReserved && tokType.keyword) || unexpected();
        node.name = tokType === _name ? tokVal : (liberal && tokType.keyword) || this.unexpected();
        this.next();
