...
define( function () {
    ...
    Parser.prototype.run = function(parsed, obj1, obj2){
        ...
        var input = {refs:args};
        this.walker.simple(parsed,handlers,null, input);
        return input.result;
    }
...
//        else if (/[89]/.test(str) || strict) this.raise(start, "Invalid number");
    ...
    Parser.prototype.readString = function(quote) {
        ...
        var out = "";
        for (;;) {
            if (tok.pos >= inputLen) this.raise(tokStart, "Unterminated string constant");
            var ch = input.charCodeAt(tok.pos);
            if (ch === quote) {
                ++tok.pos;
...
//                    if (strict) raise(tok.pos - 2, "Octal literal in strict mode");
                    ...
                    switch (ch) {
                        ...
                        case 13: if (input.charCodeAt(tok.pos) === 10) ++tok.pos; // '\r\n'
//                        case 10: // ' \n'
...
//                if (tok.type !== _parenL) unexpected();
...
//        if (!base) base = exports.base;
    ...
    Walker.prototype.Found = function Found(node, state) { this.node = node; this.state = state; }
...
//    });
    ...
    var handlers = {
        ExpressionStatement:function(node, st){
            st.result = node.expression.value;
        },
        ...
        UpdateExpression:function(node, st){
            var operator = node.operator, prefix = node.prefix, arg = node.argument, obj = arg.object.value,
                prop = arg.property.name, oldVal = arg.value, newVal = oldVal;
            switch(operator){
                case '++': newVal = ++obj[prop]; break;
                case '--': newVal = --obj[prop]; break;
                default :
                    throw new Error('update using operator '+operator+' not supported');
            }
            node.value = (prefix)? newVal : oldVal;
        },
...
//                var obj = st[node.object.name];
        ...
        CallExpression:function(node, st){
            ...
            if (st.refs){
                ...
                for (var i= 0;i<len; i++){
                    if (st.refs[i][func]){
                        realFunc = st.refs[i][func];
                        found++;
                    }
                }
            }

            if (!realFunc) throw new Error('environment has no function named '+func);
            var args = [];
            if (node.args){
            ...
            node.value = realFunc.apply(st,args);
        },
        ...
        LogicalExpression:function(node, st){
            var left = node.left.value, right = node.right.value, operator = node.operator, result = null;
            switch(operator){
