...
define( function () {
    ...
    Parser.prototype.finishOp = function(type,size){
        var tok = this.tok;
        var str = input.slice(tok.pos, tok.pos + size);
        tok.pos += size;
...
//        if (options.locations) tokStartLoc = new line_loc_t;
    ...
    Parser.prototype.readNumber = function(startsWithDot) {
        var tok = this.tok, input = this.input, inputLen = this.inputLen,
            start = tok.pos, isFloat = false, octal = input.charCodeAt(tokPos) === 48;
        ...
        if (isFloat) val = this.parseFloat(str);
        else if (!octal || str.length === 1) val = this.parseInt(str, 10);
        else if (/[89]/.test(str)) this.raise(start, "Invalid number");
//        else if (/[89]/.test(str) || strict) this.raise(start, "Invalid number");
        else val = this.parseInt(str, 8);
        return this.finishToken(_num, val);
    }
    ...
    Parser.prototype.readString = function(quote) {
        ...
        for (;;) {
            ...
            if (ch === 92) { // '\'
                ...
                while (octal && this.parseInt(octal, 8) > 255) octal = octal.slice(0, octal.length - 1);
                if (octal === "0") octal = null;
                ++tokPos;
                if (octal) {
//                    if (strict) raise(tokPos - 2, "Octal literal in strict mode");
                    out += String.fromCharCode(this.parseInt(octal, 8));
...
//                raise(tokStart, "The keyword '" + word + "' is reserved");
    ...
    Parser.prototype.startNodeFrom = function() {
        var node = new node_t();
