...
//            var id = (options && options.strategy && typeof options.strategy == 'function')? 'aggregate:'+uuid.v4() : ep.id;
    ...
    AggregatorComponent.prototype.produce = function (exchange, cb) {
        ...
        var store = this.store, ruze = this.ruze;

        var agId = -1, agIdx = -1, batchComplete = null, predComplete = null, storeSize, completeSize;
        try {
            var ex = extras(ruze,exchange);
            agId = expr.run(this.aggregateAst, exchange, ex);
            if (!store[agId]){
            ...
            if (batch){
                agIdx = expr.run(this.indexAst, exchange, ex);
                batchComplete = expr.run(this.batchAst, exchange, ex);
