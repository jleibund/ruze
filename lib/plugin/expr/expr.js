...
define(['exprjs', 'node-uuid', 'underscore'],function(expr, uuid, _) {
    ...
    var ExprComponent = function(camel,expression){
        this.camel = camel;
        this.ast = expr.parse(expression);
    }
    ExprComponent.config = function(camel,next){
        camel.mixin('expr', function(expression){
            var hc = new ExprComponent(camel,expression);
    ...
    ExprComponent.prototype.produce = function(exchange,cb){
        var out = null, err = null, endpoints = this.camel.endpoints;
