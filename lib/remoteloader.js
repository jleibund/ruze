...
define(['require', 'q', 'underscore', 'node-uuid', './remotecomponent'], function (require) {
    ...
    remoteloader.fn.getSocketId = function (socket) {
        if (!socket) return null;
        var ss = socket.socket;
        return socket.id || ss && ss.sessionid;
    }

    remoteloader.fn.getSocket = function (socketId) {
        if (_.isArray(socketId)) {
            var sockets = [];
            _.each(this.sockets, function (client, sid) {
                if (_.contains(socketId, sid)) {
                    sockets.push(this.sockets[sid]);
                }
            }, this);
            return sockets;
        }
...
//                    client.socketId = self.getSocketId(socket);
                    ...
                    var disconnectAction = function () {
                        ...
                        if (remotes) {
                            _.each(remotes, function (remote) {
                                ...
                                if (remote.sockets && _.isEmpty(remote.sockets)) {
                                    // remove the route..
                                        ...
                                        if (rebuildRoute.locallyDefined) {
                                            ...
                                            if (clientRemoteId) {
                                                ...
                                                if (clientRemote && clientRemote.sockets) {
                                                    ...
                                                    _.each(clientRemote.sockets, function (sid) {
                                                        var sock = self.getSocket(sid);
//                                                        var sock = clientRemote.sockets[sid];
                    ...
                    var connectAction = function (cb) {
                        ...
                        socket.emit(ID_CHANNEL, ruze.id, function (data) {
                            ...
                            client.ready = true;
                            client.socketId = self.getSocketId(socket);
                            self.sockets[client.socketId] = socket;
    ...
    remoteloader.fn.createServer = function (listen) {
        ...
        if (listen) {
            ...
            listen.on('connection', function (socket) {
                ...
                var socketId = self.getSocketId(socket);
                    ...
                    _.each(incoming, function (subroute) {
                        ...
                        if (subroute.route && subroute.route.length) {
                            ...
                            if (!existing) {
                                ...
                                var entryRC = new remotecomponent({ruze: ruze, loader: self, from: callbackEp, next: subroute.route[0], route: subroute, terminate: callbackRoute.terminate, parent: subroute.id, client: false});
                                // add this as a new socket to use
                                entryRC.addSocket(socketId);
                                ...
                                existing.instance.addSocket(socketId);
                                //existing.refCount++;
                                if (!self.endpointsBySocket[socketId]) self.endpointsBySocket[socketId] = {};
...
//        return Q.allResolved(requests).then(function () {
        ...
        var allResolvedOrSettled = Q.allSettled || Q.allResolved;
        return allResolvedOrSettled(requests).then(function () {
            return strategy;
        })
...
//                            var rc = new remotecomponent({ruze:ruze, from:subroute.route[0], next:subroute.next, route:subroute, client:true});
                        var rc = new remotecomponent({ruze: ruze, loader: self, from: subroute.route[0], next: subroute.next, route: subroute, parent: r.id, client: true});
                        // add it to computed
            ...
            var promise = this.ready({name: req.container, url: req.channel})
                .then(function (client) {
                    ...
                    _.each(req.rcs, function (rc) {
                        // todo: mixup on sockets here
//                        rc.addSocket(client.socket, strategy.client || socketId);
                        rc.addSocket(socketId);
