    ...
    function (require) {
        ...
        remotecomponent.fn.produce = function (exchange, cb) {
            ...
            var sockets = (!clone.properties.broadcast && clone.properties.socket &&
                this.sockets[clone.properties.socket]) ? [this.sockets[clone.properties.socket]] : this.sockets;
                ...
                listen.on('connection', function (socket) {
                    ...
                    socket.on(CREATE_CHANNEL, function (data, fn) {
                        ...
                        if (!_.isEmpty(toProcess)) {
                            ...
                            var strategy = {client:socket.id, routes: toProcess, components: ruze._getComponents(_.values(toProcess))};
                        ...
                        if (channel) {
                            ...
                            var promise = this.ready({name: proxyContainer, url: channel})
                                .then(function (client) {
                                    ...
                                    // set the route id
                                    subroute.id = r.id;
                                    if (subidx == r.route.length-1) subroute.terminate = true;
