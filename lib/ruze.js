...
define([ 'require', 'module', 'q', 'node-uuid', 'events', 'underscore', 'colors', 'path', 'cutils', 'exprjs'],
    function (require) {
        ...
        var remotecomponent = function (ruze, from, next, rte, terminate) {

            this.sockets = {};
            this.ruze = ruze;
            this.next = next;
            this.route = rte;
            this.endpoint = ruze.parseEndpoint([remotecomponent.genId(rte)]);
            this.from = from;
...
//                    if (self.next && self.endpoint.id == data.endpoint.id) {
                        e.out = e.in;
                        ...
                        //todo - need to replace this call with emitRecipients
                        ...
                        _.each(incoming, function (subroute) {
                            ...
                            if (subroute.route && subroute.route.length) {
                                ...
                                if (!existing) {
                                    ...
                                    var entryRC = new remotecomponent(ruze, callbackEp, subroute.route[0], subroute, callbackRoute.terminate);
...
//                        client.socket.connect(client.url);
                        ...
                        if (channel) {
                            ...
                            var promise = this.ready({name: proxyContainer, url: channel})
                                .then(function (client) {
                                    ...
                                    _.each(payload.routes, function(rte){
                                        if (!rte.next) rte.terminate = true;
                                    })
                                ...
                                if (!exchange) {
                                    ...
                                    if (instance.produce) {
                                        instance.produce(exchange, function (err, e) {
                                            ...
                                            self.emitRecipients(cur.config,exchange);
        ...
        ruze.fn.emitRecipients = function (ep, exchange) {
            var ruze = this;
            // exchange overrides
            ...
            if (rl){
            ...
            } else if (endpoint && endpoint.process && endpoint.process.length){
                ...
                var keys = endpoint.process || [];

                // kick off any other this node has
                var diff = _.difference(keys, e.done);
                if (diff && diff.length) {
