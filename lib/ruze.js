    ...
    ruze.fn._buildRecipientsAndEvents = function(routes){
        ...
        _.each(routes, function(route){
            for (var i=0; i< route.route.length; i++){
                var curEp = route.route[i];
                var cur = this.getEndpoint(curEp);
                var prevEp = (i>0)? route.route[i-1] : null;
                if (prevEp) {
                ...
                if (!listeners || !listeners.length) {
                    self.events().on(curEp.id, function (exchange) {
                        if (!exchange) {
                            if (instance.consume) {
                                instance.consume(function (err, e) {
                                    if (err) throw err;
                                    e.fromEndpoint = _.clone({},curEp);
                                    self.emitRecipients(curEp, e);
                                });
                            }
                        } else {
