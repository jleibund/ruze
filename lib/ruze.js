...
define([ 'require', 'q','node-uuid', 'events', 'underscore', 'cutils', 'colors', 'exprjs', 'module', 'path'],
//    function (uuid, events, _, Q, cutils, colors, exprjs, module, path) {
    function (require) {
        ...
        var uuid = require('node-uuid'), events = require('events'), _ = require('underscore'),
            cutils = require('cutils'), Q = require('q'), colors = require('colors'), exprjs = require('exprjs'), module = require('module'),
...
//                    newRoutes = definition;
            ...
            if (!loadall || !promise) {
                ...
                promise = this._load(strategy)
                    .then(function () {
                        return self._configure(strategy).then(function(){return {}});
                    })
                this.loadall = true;
                if (definition) {
        ...
        ruze.fn.start = function (route1, route2, more) {
            ...
            var start = _.bind(function (routesToStart) {
            ...
            return this.configuration.then(
                function (routes) {
                    ...
                    var strategy = {routes:unBuilt, components:self._getComponents(_.values(unBuilt))};
                    ...
                    return self._load(strategy).then(
                        ...
                        function (strategy) {
                            ...
                            _.each(strategy.routes, function(v,k){
                                ...
                                // either way lets add some other pieces
                                if (strategy.strategy[k]){
                                    v.strategy = strategy.strategy[k];
                                }
                                if (strategy.computed[k]){
                                    v.computed = strategy.computed[k];
                                }
                            })
                            return deltaToStart;
                        })
