...
define([ 'require', 'module', 'q', 'node-uuid', 'events', 'underscore', 'colors', 'path', 'cutils', 'exprjs'],
    function (require) {
        ...
        remotecomponent.fn.addSocket = function (socket) {
            var ruze = this.ruze, self = this, id = socket.id;
            if (socket && !_.contains(_.keys(this.sockets), id)) {
                ...
                socket.on(PROCESS_CHANNEL, function (data) {
                    ...
                    if (data.endpoint.id == self.from.id && self.next) {
                        ...
                        if (e.process && e.process.length && self.endpoint.id != e.process[0].id){
                            // push current state into the exchange
                            if (!e.parent) e.parent = [];
                            e.parent.unshift({process: e.process, routeId: e.routeId});
                            delete e.process;
                            delete e.routeId;
                        } else {
                            if (e.process && e.process.length) e.process.shift();
                            delete e.parent;
                        }

                        // fire the exchange after doing in the expected in/out swap.  this is the kick off call for the route
                        ruze.events().emit(self.next.id, e);
                    }
                })
            }
        }

        remotecomponent.fn.removeSocket = function (socket) {
            var ruze = this.ruze, self = this, id = socket.id;
            if (socket && _.contains(_.keys(this.sockets), id)) {
        ...
        remotecomponent.fn.produce = function (exchange, cb) {
            ...
            // push current state into the exchange
            if (clone.parent) {
                var state = clone.parent.shift();
                clone.process = state.process;
                clone.routeId = state.routeId;
            }

            // grab the sockets-- if we're filtered by client (see above) we grab just its socket
            var sockets = (!clone.properties.broadcast && clone.properties.client &&
                        ...
                        _.each(incoming, function (subroute) {
                            ...
                            if (subroute.route && subroute.route.length) {
                                ...
                                if (!existing) {
                                    ...
                                    entryRC.addSocket(socket);
                                    if (!self.endpointsBySocket[socket.id]) self.endpointsBySocket[socket.id] = [];
                                    self.endpointsBySocket[socket.id].push(entryRC);
                                    ...
                                    existing.instance.addSocket(socket);
                                    if (!self.endpointsBySocket[socket.id]) self.endpointsBySocket[socket.id] = [];
                                    self.endpointsBySocket[socket.id].push(existing.instance);
                                }
                            }
                        }, this);
...
//                            cb(e, client);
                        ...
                        socket.on('reconnect', function () {
                            ...
                            socket.emit(ID_CHANNEL, ruze.id, function (data) {
                                ...
                                // todo: need to rebuild all of the needed routes.. or would this happen automagically?
                                ...
                                if (!exchange) {
                                    if (instance.consume) {
                                        ...
                                        instance.consume(function (err, e) {
                                            ...
                                            _.each(_.keys(cur.process), function (routeId) {
                                                var cloneEx = _.extend({}, e);
                                                cloneEx.routeId = routeId;
                                                var process = cur.process && cur.process[routeId];
                                                if (process && process.length)
                                                    cloneEx.process = process.slice();
                                                cloneEx.process.shift();
                                                self.emitRecipients(cloneEx);
                                            })
                                        });
                                    }
                                } else {
                                    ...
                                    if (instance.produce) {
                                        instance.produce(exchange, function (err, e) {
                                            ...
                                            if (!e.process || !e.process.length) {
                                                // mark done
                                                if (!e.done) e.done = [];
                                                if (e.process && !_.contains(e.done, id)) e.done.push(id);

                                                // kick off any other this node has
                                                var diff = _.difference(_.keys(cur.process), e.done);
                                                if (diff && diff.length) {
                                                    _.each(diff, function (routeId) {
                                                        var cloneEx = _.extend({}, e);
                                                        cloneEx.fromEndpoint = _.clone({}, cur.config);
                                                        var process = cur.process && cur.process[routeId];
                                                        if (process && process.length)
                                                            cloneEx.process = process.slice();
                                                        cloneEx.process.shift();
...
//                        prevEp = curEp;
        ...
        ruze.fn.emitRecipients = function (exchange) {
            var ruze = this;
            // exchange overrides
            //todo:  recipient list needs to know what route its on -- in the exchange
            var rl = (exchange.out && exchange.out.recipientList) ? exchange.out.recipientList : exchange.process && exchange.process.length && exchange.process[0];
            if (rl) {
        ...
        ruze.fn.send = function (endpoint, body, cb) {
            ...
            ex.out.body = body;
//            var wrapper = this.getEndpoint(ep.id);
//            _.each(_.keys(wrapper.process), function(routeId){
//                var cloneEx = _.extend({},ex);
//                cloneEx.routeId = routeId;
//                var process = wrapper.process && wrapper.process[routeId];
//                if (process && process.length)
//                    cloneEx.process = process.slice();
//                this.emitRecipients(cloneEx);
//            },this)
//
