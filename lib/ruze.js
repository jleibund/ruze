...
//        }
    ...
    localloader.prototype._load = function (strategy, cb) {
            var self = this;
        ...
        localloader.prototype.configure = function(strategy){
        ...
        localloader.prototype._create = function(strategy,cb) {
            var self = this, ruze = this.ruze, result = {};
    ...
    var remotecomponent = function (ruze, container, next) {
...
//        this.endpoints = {};
        ...
        this.id = uuid.v4();
        this.endpoint = ruze.parseEndpoint(container+':remote:'+this.id);
    ...
    remotecomponent.fn.addSocket = function(id,socket){
        ...
        if (!_.contains(_.keys(this.sockets),id)){
            ...
            socket.on(PROCESS_CHANNEL,function(data){
                ...
                if (data.endpoint.id == self.next.id)
                    data.exchange.out = data.exchange.in;
                    ruze.emitRecipients(self.next, data.exchange);
            })
        }
    }
...
//            var container = ep.container;
        ...
        this.configure = function(strategy){
                    ...
                    _.each(r.route, function(subroute){
                        ...
                        if (channel){
                            ...
                            var promise = this.ready({name:container,url:channel})
                                .then(function(client){
                                    var deferred = Q.defer();
                                    client.socket.emit(CREATE_CHANNEL,{id:ruze.id, request:subroute},function(data){
                                        var rc = new remotecomponent(ruze, subroute.next);
                                        rc.addSocket(client.id, client.socket);
                                        sub.push(rc.endpoint);
                                        result[rc.endpoint.id] = rc;
                                        // replace in computed
    ...
    ruze.fn.configure = function (definition) {
        ...
        var strategy = {components:this.preload, configure:{local:this.preload}};
        if (!loadall || !promise) {
            var self = this;
    ...
    ruze.fn._bindStrategy = function(strategy){
        ...
        _.each(strategy.routes, function(route){
            var prevEp = null;
            var i = 0;
            var runs = {id:route.id, route:[]}, curRun;
            var placeholder = [];
            strategy.computed[route.id] = placeholder;
            _.each(route.route, function(sequence){
                var curEp = sequence;
                ...
                if (curContainer){
                    ...
                    if (curContainer != priorContainer){
                        var next = (curRun) ? _.last(curRun).id : null;
                        curRun = [];
                        runs.route.push({container:curContainer, route:curRun, next:next})
                        placeholder.push([]);
                    }
                    ...
                    var next = (curRun) ? _.last(curRun).id : null;
                    curRun = [];
                    runs.route.push({container:bestContainer, route:curRun, next:next})
                    placeholder.push([]);
    ...
    ruze.fn._doLoaders = function(fn,arg,local){
        ...
        _.each(this.loaders, function(loader){
            promise = (!promise) ? loader[fn](arg) : promise.then(function(obj){ return loader[fn](obj); });
        })
    ...
    ruze.fn.start = function (route1, route2, more) {
        ...
        return this.configuration.then(function () {
            ...
            var routes = {};
            _.each(args,function(a){routes[a.id]=a});

            var strategy = {routes:routes, components:self._getComponents(args)};

            return self._load(strategy).then(function(strategy){
