                    ...
                    io.on('connection', function (socket) {
                        ...
                        socket.on(CREATE_CHANNEL, function (data, fn) {
                            ...
                            if (subroute.route && subroute.route.length){
                                ...
                                if (existing){
                                    fn({id:ruze.id, route:existing.config.id})
                                } else {

                                    var localRoute = new route(ruze);

                                    _.each(subroute.route, function(ep){
                                        var local = _.extend({}, ep);
                                        local.container = 'local';
                                        ruze.rewriteEndpointId(local);
                                        localRoute.route.push(local);
                                    },this)

                                    var entryRC = new remotecomponent(ruze, subroute.route[0],localRoute.route[0], subroute);
                                    entryRC.addSocket(data.id, socket);
                                    entryRC.local = localRoute;
                                    localRoute.route.push(entryRC.endpoint);
                                    self.endpoints[entryRC.endpoint.id] = {config:entryRC.endpoint, instance:entryRC};


                                    ruze.start(localRoute, function(){
                                        fn({id:ruze.id, route:localRoute.id})
                                    });

                                }
                            } else {
                                fn({id:ruze.id, route:null})
                            }
                        })
