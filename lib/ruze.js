...
//        }
    ...
    localloader.prototype._load = function (componentList, cb) {
        ...
        localloader.prototype._create = function(strategy,cb) {
            ...
            var components = this.components;
            _.each(strategy.strategy, function(r){
...
//                                    .then(promise).then().done();
    ...
    ruze.fn.configure = function (definition) {
        ...
        if (!loadall || !promise) {
            var self = this;
            promise = this._load(self.preload,true)
                .then(function(){
                    return self._configure({configure:{local:self.preload}},true)
                });
            this.loadall = true;
        }
        var self = this;
    ...
    ruze.fn._load = function (components, local) {
        return this._doLoaders('load',_.uniq(components),local).then(function(loads){
            // this is the beginning of our 'strategy' object.
            ...
            _.each(route.route, function(sequence){
                ...
                if (curContainer){
                    ...
                    if (curContainer != priorContainer){
                        curRun = [];
                        runs.route.add({container:curContainer, route:curRun})
                    }
    ...
    ruze.fn._configure = function (strategy, local) {
        if (local){
            return this.loaders.local.configure(strategy.configure.local)
        }
        return this.loaders.local.configure(strategy.configure.local).then(function(){
            return strategy;
        })
    };
    ...
    ruze.fn.start = function (route1, route2, more) {
        ...
        return this.configuration.then(function () {
            ...
            var components = [];
            _.each(args, function(r){
                console.log(r)
                components = (components.length)? _.union(r._getComponents(), components) : r._getComponents();
            })


            return self._load(components).then(function(strategy){
                    strategy.routes = args;
                    return strategy;
    ...
    ruze.fn.endpoint = function (end, cb, instance, recipients) {
        ...
        if (self.getEndpoint(parsed.id)) {
            ...
            return self._load(r._getComponents()).then(function(strategy){
                    strategy.routes = [r];
                    return strategy;
                }).then(function(strategy){
                    return self._bindStrategy(strategy);
                    ...
                    return self._create(strategy);
                }).then(self._create).then(function(){
