            ...
            if (listen) {
                ...
                listen.on('connection', function (socket) {
                    ...
                    socket.on(CREATE_CHANNEL, function (data, fn) {
                        ruze.log('create', CREATE_CHANNEL)
                                ...
                                _.each(remotes, function(remote){
                                    ...
                                    if (remote.sockets && _.isEmpty(_.keys(remote.sockets))) {
                                        ...
                                        if (remote.route.locallyDefined){
                                            ...
                                            var remoteRoute = remote.route;
                                            var remoteRouteId = remoteRoute.id;

                                            if (!self.rebuild[remoteRouteId]){
                                                var rebuildRoute = ruze.routes[remoteRouteId];
                                                rebuildRoute.built = false;
                                                delete rebuildRoute.sequence;
                                                delete rebuildRoute.strategy;
                                                delete rebuildRoute.computed;
                                                self.rebuild[remoteRouteId] = rebuildRoute;
//                                                ruze.routes[remoteRouteId] = null;
                                            }
//                                            ruze._removeRoute(remoteRoute);
                                        }
                                    }
                                })
                        ...
                        socket.on('reconnect', function () {
                            ...
                            if (!_.isEmpty(self.rebuild)){
                                self.ruze.start();
//                                self.ruze.start.apply(ruze, _.values(self.rebuild));
//                                self.rebuild = {};
                            }
                        });
                    } else {
...
//                            var rc = new remotecomponent({ruze:ruze, from:subroute.route[0], next:subroute.next, route:subroute, client:true});
                            var rc = new remotecomponent({ruze:ruze, from:subroute.route[0], next:subroute.next, route:r, client:true});
                            // add it to computed
